# Theme: Catppuccin Mocha (pink accent)
# Catppuccin Mocha with pink/rosewater promoted to primary.

# ─── Semantic ───
THEME_BG="#1e1e2e"
THEME_BG_ALT="#181825"
THEME_SHADOW="#11111b"
THEME_FG="#cdd6f4"
THEME_PRIMARY="#f5c2e7"     # pink
THEME_SECONDARY="#cba6f7"   # mauve
THEME_TERTIARY="#89b4fa"    # blue
THEME_ACCENT="#f5e0dc"      # rosewater
THEME_HOVER="#f2cdcd"       # flamingo
THEME_SURFACE="#313244"
THEME_OUTLINE="#45475a"
THEME_SUCCESS="#a6e3a1"
THEME_WARNING="#f9e2af"
THEME_ERROR="#f38ba8"
THEME_ORANGE="#fab387"

# ─── ANSI 16 ───
THEME_BLACK="#45475a";   THEME_BLACKB="#585b70"
THEME_RED="#f38ba8";     THEME_REDB="#f38ba8"
THEME_GREEN="#a6e3a1";   THEME_GREENB="#a6e3a1"
THEME_YELLOW="#f9e2af";  THEME_YELLOWB="#f9e2af"
THEME_BLUE="#89b4fa";    THEME_BLUEB="#89b4fa"
THEME_MAGENTA="#f5c2e7"; THEME_MAGENTAB="#f5c2e7"
THEME_CYAN="#94e2d5";    THEME_CYANB="#94e2d5"
THEME_WHITE="#bac2de";   THEME_WHITEB="#a6adc8"

# ─── Metadata ───
THEME_DISPLAY_NAME="Catppuccin Pink"
THEME_GHOSTTY_BUILTIN="Catppuccin Mocha"
THEME_NVIM_COLORSCHEME="catppuccin"
THEME_NVIM_STYLE="mocha"
