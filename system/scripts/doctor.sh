...
_header() {
  _rule
  printf "${CYAN}${BOLD}  🛠  Dotfiles Health Check${NC}\n"
  _rule
  echo
}
...
if [[ -f "$DOTFILES_DIR/system/themes/generated/.current-theme" ]]; then
  ...
  _pass "Theme active: $current_theme"
  ((passed++)) || true
else
  _warn "No theme applied"
fi
...
_rule
if [[ $failed -eq 0 ]]; then
  printf "${GREEN}${BOLD}  ✓  All $passed checks passed${NC}\n"
else
  printf "${RED}${BOLD}  ✗  $failed check(s) failed out of $((passed + failed))${NC}\n"
  exit 1
fi
_rule
